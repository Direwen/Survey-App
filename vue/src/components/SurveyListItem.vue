<template>
    <div class="flex flex-col py-4 px-6 shadow-md bg-white hover:bg-gray-50 h-[470px]">
        <img :src="survey.image_url" alt="Survey Image" class="w-full h-48 object-cover">
        ...
        <div class="flex justify-between items-center mt-3">
            ...
            <router-link :to="{ name: 'SurveyView', params: { id: survey.id } }"
                class="flex justify-between items-center py-2 px-4 border border-transparent text-sm rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
            ...
            <div class="flex items-center">
                ...
                <button v-if="survey.id" type="button" @click="emit('delete', survey)"
                    class="h-8 w-8 flex justify-center items-center rounded-full border border-transparent text-sm text-red-500 focus:ring-2 focus:ring-offset-2 foucs:ring-red-500">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
